...
export default async function OrgScansPage({ params, searchParams }: Params)
{
	...
	// 	{
	// 		if (scan.status === RequestStatus.ACCEPTED)
	// 			trendMap[dateString].accepted++;
	// 		if (scan.status === RequestStatus.REJECTED)
	// 			trendMap[dateString].rejected++;
	// 	}
	// }
	...
	const demo_analytics =
	{
		...
		trendData:
		[
			{ date: "2026-06-18", accepted: 60, rejected: 5 },
			{ date: "2026-06-19", accepted: 75, rejected: 6 },
			{ date: "2026-06-20", accepted: 80, rejected: 7 },
			{ date: "2026-06-21", accepted: 65, rejected: 4 },
			{ date: "2026-06-22", accepted: 90, rejected: 3 },
			{ date: "2026-06-23", accepted: 70, rejected: 5 },
			{ date: "2026-06-24", accepted: 50, rejected: 5 },
		],
	};
	const demo_days = 7;
	const demo_name = "Asmae";

	return (
		<StatisticsClient analytics={demo_analytics} periodDays={demo_days} orgID={orgId} orgName={demo_name}/>
	);
}
