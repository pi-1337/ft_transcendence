...
export default async function OrgScansPage({ params, searchParams }: Params) {
  ...
  const trendMap: Record<
    string,
    { date: string; accepted: number; rejected: number }
  > = {};
  ...
  const analytics = {
    ...
    meals: org.meals.map((meal) => {
      const accepted =
        mealStats.find(
          (s) => s.mealId === meal.id && s.status === RequestStatus.ACCEPTED,
        )?._count._all ?? 0;
      const rejected =
        mealStats.find(
          (s) => s.mealId === meal.id && s.status === RequestStatus.REJECTED,
        )?._count._all ?? 0;
      return {
        id: meal.id,
        name: meal.name,
    ...
    totalAccepted: mealStats
      .filter((s) => s.status === RequestStatus.ACCEPTED)
      .reduce((sum, s) => sum + s._count._all, 0),
    totalRejected: mealStats
      .filter((s) => s.status === RequestStatus.REJECTED)
      .reduce((sum, s) => sum + s._count._all, 0),
    trendData,
  };
