...
import { 
    ...
    KeyRound, ShieldCheck, XCircle, AlertCircle, CheckCircle2 
} from "lucide-react";
...
export default function Settings({ user }: { user: UserFrontend }) {
    ...
    const requestTwoFactorChange = async (target: "enable" | "disable") => {
        ...
        try {
            ...
            setTwoFactorStatus(`Code sent to ${data.maskedEmail || 'your email'}. Enter it below to confirm.`);
        } catch (err: any) {
        ...
        <div className="min-h-screen bg-gray-950 text-white pb-12">
            ...
            <main className="max-w-2xl mx-auto px-6 mt-10 space-y-8">
                <div>
                    ...
                    <p className="text-sm text-gray-400">Manage your personal information and security preferences.</p>
                </div>
                ...
                <Card className="bg-gray-900 border-gray-800">
                    <CardHeader className="pb-4 border-b border-gray-800">
                        <CardTitle className="text-gray-200 text-lg font-semibold flex items-center gap-2">
                            <KeyRound className="w-4 h-4 text-green-500" /> Two-Factor Authentication
                        </CardTitle>
                    </CardHeader>
                    ...
                    <CardContent className="pt-6 space-y-5">
                        <div className="flex items-center justify-between p-4 bg-gray-950/50 border border-gray-800 rounded-lg">
                            <div>
                                <p className="text-white text-sm font-medium flex items-center gap-2">
                                    Status: 
                                    {twoFactorEnabled ? (
                                        <span className="text-green-400 flex items-center gap-1"><ShieldCheck className="w-4 h-4"/> Enabled</span>
                                    ) : (
                                        <span className="text-gray-400 flex items-center gap-1"><XCircle className="w-4 h-4"/> Disabled</span>
                                    )}
                                </p>
                        ...
                        {!twoFactorPendingAction ? (
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-medium text-gray-400">Current Password (Required)</label>
                                    <Input
                                        type="password"
